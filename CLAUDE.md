...
- **[app/studio/page.js](app/studio/page.js)** — the mockup studio frontend (one big `"use client"` component) at route `/studio`. Holds the method/view/placement/size catalogs and the `STRINGS` i18n dictionary. Two `UploadZone`s (logo + product photo), a method segmented control, and a multi-select of shot types (`views`). **Generation history is an append-only list of immutable `batches`** — each Generate run captures the two images + settings it used and is never mutated, so changing the inputs and regenerating never destroys earlier results. It calls **only** `/api/generate-mockups`, never OpenRouter directly (this is what keeps the API key server-side). **Its work-in-progress state (uploaded images, settings, drawn circle, and `batches`) does NOT live here** — it comes from `useStudioState()` (see below); only ephemeral UI state (open modals, current error, lightbox) is local `useState`.

- **[app/StudioStateContext.js](app/StudioStateContext.js)** — a `"use client"` React context (`StudioStateProvider` + `useStudioState`) mounted in the root layout, holding the studio's uploaded logo/product images, settings, drawn placement `marker`, `batches`, and the `batchSeq` id counter. It lives at the layout level so navigating away from `/studio` (to `/simplify` or `/`) and back does **not** wipe the in-progress work — the root layout persists across client-side `next/link` navigation, so the provider never unmounts. State is in-memory (not `sessionStorage`), so a **full page reload does reset it** — deliberate, because the generated images are large base64 blobs that would blow past storage quotas.
...
- **[app/colorUtils.js](app/colorUtils.js)** — shared browser-side color module (`"use client"`-safe, no React). Owns the k-means quantizer (`quantizeImage`, `seedCentroids`, `mulberry32`, `svgTargetDims`, `rgbToHex`) used by `ColorSimplifier.js`, plus two extraction helpers used by the studio's color-consistency pipeline (see below): `extractPalette(dataUrl, k)` — the logo's dominant colors as a deduplicated `{hex, share}[]` — and `sampleRegionColor(dataUrl, region)` — the average color of a region of the product photo (the drawn marker circle, or a central crop as fallback).

- **[app/ColorSimplifier.js](app/ColorSimplifier.js)** — self-contained `"use client"` k-means color quantizer UI (canvas `getImageData` → cluster via `colorUtils.js` → snap pixels → `toDataURL`). Deterministic (seeded PRNG) so the same image + color count always yields the same palette. No server, no API cost. Carries its own copy of the `STRINGS` i18n dict. Rendered only by `app/simplify/page.js`.
...
- **[app/layout.tsx](app/layout.tsx)** — root layout, Geist fonts, metadata. Wraps all routes in `StudioStateProvider` (a client component) so studio state survives cross-route navigation. **[app/globals.css](app/globals.css)** — Tailwind v4 (`@import "tailwindcss"`, `@theme inline`), the `--background`/`--foreground` theme tokens, dark mode via `prefers-color-scheme`, and the `animate-fade-up` / `skeleton-shimmer` utilities the components reference.
...
- **Deterministic color spec, not per-shot judgment** — this is what keeps the logo's color identical across shots. The studio page extracts the logo's exact hex palette (`extractPalette`) and samples the product's color where the logo will sit (`sampleRegionColor`), both from `app/colorUtils.js`, and sends them as `logoColors` / `productColor`. The backend's `colorSpecClause(logoColors, productColor)` computes a WCAG-contrast-ratio substitution **once, in code** (not per-shot, not by the model) and hands every shot of the request the identical closed hex-to-hex table — there's nothing left for the model to decide differently between shots. `CONTRAST_ADAPTATION` (text-only, "you decide") is kept only as a fallback for when the client couldn't extract a palette (e.g. a canvas read failure); when active it's the older mechanism and is more prone to the cross-shot drift `colorSpecClause` exists to fix.
- **Chained-generation-for-consistency** (`generateShotSet`): the **first requested view** (in canonical `VIEW_ORDER`, so a closeup+model run anchors on the close-up) is generated first as the anchor, then its rendered image is passed as the *last reference image* to every other requested shot, with `DECORATED_MATCH` telling the model to copy that exact decorated item — including treating its logo colors as authoritative rather than re-deriving them. If the anchor shot fails, the rest fall back to their base references (logo + product [+ marker]) and run in parallel.
